...
function replaceText(word, replacement, text) {
	const repl = new RegExp(word, 'g'); 
    const newText = text.replace(repl, replacement); 
    return newText; 
...
function replaceWordOccurrences(str, word, newWord) {
	const redexps = new RegExp(word, "g")
	return str.replace(redexps, newWord);
